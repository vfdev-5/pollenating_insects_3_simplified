...
class ImageClassifier(object):
    ...
    def _save_second_level_model(self):
        second_level_model_filename = os.path.join(self.logs_path, "second_level_model.xgb")
        self.second_level_model.save_model(second_level_model_filename)

    def _predict_second_level(self, x):
        second_level_model_filename = os.path.join(self.logs_path, "second_level_model.xgb")
        assert os.path.exists(second_level_model_filename)
        print("Load second level model found at %s" % second_level_model_filename)
        self.second_level_model = xgb.Booster(model_file=second_level_model_filename)

        # Select top-3 predicted classes:
        y_topk_preds = self._compute_topk(x, k=3)
        dtest = xgb.DMatrix(y_topk_preds)
        y_new_pred = self.second_level_model.predict(dtest)
        return y_new_pred
    ...
    def fit(self, img_loader):
        ...
        # Train second level
        self._train_second_level(y_probas_1lvl, img_loader.y_array)

    def predict_proba(self, img_loader):
        y_probas_1lvl = self._predict_first_level(img_loader)
        y_preds = self._predict_second_level(y_probas_1lvl)
